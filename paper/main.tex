...
\title{PhyPro: Learning When Physics Should Promote Correction for Robust Traffic State Reconstruction}
...
\noindent\textbf{Anonymous code:} \url{https://anonymous.4open.science/r/PhyGuard-F1CE}
...
Traffic reconstruction has benefited from both structured statistical priors and deep spatiotemporal modeling. Tensor completion and temporal factorization methods exploit low-rank and temporal regularities in traffic measurements \citep{chen2020nonconvex,chen2021scalable,chen2022latc}. Deep graph and attention models further capture nonlinear temporal dynamics and road-network dependencies, as shown in traffic forecasting and spatiotemporal modeling studies \citep{li2018dcrnn,yu2018stgcn,wu2019graphwavenet,zheng2020gman}. For missing data, recurrent, self-attention, diffusion, and graph neural imputation methods have become strong reconstruction choices \citep{cao2018brits,du2023saits,tashiro2021csdi,cini2022grin}. Recent traffic-oriented imputation models also introduce mask-aware graph repair and low-rank transformer structures \citep{liu2025maginet,liu2024imputeformer}. These methods provide strong backbones, but they mainly answer how to reconstruct a value. They do not directly answer whether a local correction to a strong backbone output is physically reliable.

Physics-informed learning offers a different source of evidence. The broad idea is to constrain learning by physical residuals or domain equations \citep{raissi2019pinn}. In traffic modeling, physical constraints can represent flow-density-speed consistency, conservation relations, graph smoothness, or macroscopic traffic dynamics \citep{shi2021pidl,tang2024pimlcalibration,wang2024pistgnet}. Such priors are appealing because sparse observations alone may not identify the local traffic state. Yet the same priors can be risky in open road networks. A simplified residual that is meaningful in normal traffic can become unreliable around failed sensors, local incidents, non-periodic congestion, or speed-only datasets without explicit flow-density information. Fixed physics losses therefore suffer from a local physical-misguidance problem: they treat physics as equally trustworthy across all sensors and time steps, even when the physical relation is locally mismatched.
...
Traffic data imputation has long exploited the structured nature of road-network measurements. Low-rank tensor and temporal factorization models recover missing values by assuming that traffic measurements share spatial, temporal, and variable-level dependencies \citep{chen2020nonconvex,chen2021scalable,chen2022latc}. Deep models extend this view by learning nonlinear dependencies from incomplete sequences and graphs. BRITS performs bidirectional recurrent imputation and treats missing values as differentiable variables in temporal state propagation \citep{cao2018brits}. SAITS uses self-attention to model temporal dependencies without recurrent states \citep{du2023saits}. CSDI introduces score-based probabilistic imputation for time series \citep{tashiro2021csdi}, while GRIN uses graph neural networks to fill gaps in multivariate time series \citep{cini2022grin}. For spatiotemporal traffic data, ImputeFormer introduces low-rankness-induced transformers for generalizable imputation, and MagiNet designs a mask-aware graph imputation network for incomplete traffic data \citep{liu2024imputeformer,liu2025maginet}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/figure_phypro_mechanism.png}
...
\begin{equation}
  ...
  C_{t,i}\in[0,1],
\end{equation}
where $C_{t,i}$ combines failure score, residual rank, observed-error proxy, and spatial-gap rank. This mechanism is the central difference between PhyPro and a generic adapter. A generic adapter learns how to correct; PhyPro additionally learns when physics should promote that correction and when it should be suppressed.
...
\begin{algorithm}[t]
\caption{PhyPro inference}
\label{alg:phypro}
\begin{algorithmic}[1]
\Require Incomplete observation $X^{obs}$, mask $M$, graph $A$, trained backbone $B_{\theta}$
\State $X_0 \gets B_{\theta}(X^{obs},M,A)$
\For{each sensor-time pair $(t,i)$}
  \State Build local evidence $\mathbf{e}_{t,i}$ from mask, temporal gap, failure score, residual rank, and conflict cues
  \State Compute generic correction $\Delta^g_{t,i}=b\tanh(f_g(\mathbf{z}^{g}_{t,i}))$
  \State Compute physics direction $\Delta^p_{t,i}=b\tanh(0.6d^{graph}_{t,i}+0.4d^{temp}_{t,i})$
  \State Estimate correction gate $g_{t,i}$ and promotion weight $\beta_{t,i}$
  \State $\hat{x}_{t,i}\gets x_{0,t,i}+g_{t,i}\Delta^g_{t,i}+\beta_{t,i}\Delta^p_{t,i}$
\EndFor
\State \Return $\hat{X}$ and local reliability/promotion maps
\end{algorithmic}
\end{algorithm}
...
PhyPro is attached to four strong and diverse reconstruction backbones: BRITS, SAITS, ImputeFormer, and MagiNet. BRITS represents bidirectional recurrent imputation, SAITS represents self-attention-based temporal imputation, ImputeFormer represents transformer-style masked spatiotemporal imputation, and MagiNet represents mask-aware graph repair. This selection avoids testing only minor variants of one architecture family.

We report two types of comparisons. The first compares each backbone with the same backbone enhanced by PhyPro. The second compares PhyPro with plug-in baselines under the same backbone outputs: Generic Adapter, Calibration Guard, and Failure/Anomaly Guard. This protocol isolates whether PhyPro improves because of physics-reliability promotion rather than because an additional adapter was added.
...
\end{equation}
RMSE and MAPE are recorded but not used for the main ranking. PhyPro uses 23,365 trainable parameters, correction bound $b=0.20$, gate floor $g_{\min}=0.95$, and conflict coefficient $\lambda_c=0.75$ unless otherwise stated.
...
\caption{Main masked MAE results. Lower is better. The best result in each row is bolded and the second-best result is underlined. B+PP, IF+PP, M+PP, and S+PP denote BRITS, ImputeFormer, MagiNet, and SAITS enhanced by PhyPro.}
\label{tab:main_mae}
\resizebox{\textwidth}{!}{%
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llcccccccc@{}}
\toprule
Dataset & Scenario & BRITS & B+PP & IF & IF+PP & MagiNet & M+PP & SAITS & S+PP \\
\midrule
...
\begin{table}[t]
\centering
\caption{Plug-in comparison over 180 paired runs. Values are mean masked MAE. Lower is better.}
\label{tab:plugin}
\begin{tabular}{lcc}
\toprule
Method & Masked MAE & Runs \\
\midrule
Backbone & 0.3804 $\pm$ 0.1881 & 180 \\
Generic Adapter & 0.3496 $\pm$ 0.1830 & 180 \\
Calibration Guard & 0.3556 $\pm$ 0.1843 & 180 \\
Failure/Anomaly Guard & 0.3550 $\pm$ 0.1841 & 180 \\
PhyPro & \textbf{0.3441 $\pm$ 0.1805} & 180 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
PhyPro vs Backbone & 10.58\% & $2.29{\times}10^{-54}$ & $2.73{\times}10^{-31}$ \\
...
\begin{table}[t]
\centering
\caption{Selected ablation results on PEMS08. Internal diagnostic variants are omitted from the main text.}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
...
Full PhyPro & \textbf{0.2786} & 0.9566 & 0.4872 \\
...
\begin{tabular}{ccccc}
\toprule
Gate floor & Conflict coefficient & Masked MAE & Gate mean & Promotion mean \\
\midrule
...
Figure~\ref{fig:case_study} visualizes a representative PEMS-BAY case under random missing. The panels show the initial backbone reconstruction, PhyPro reconstruction, error reduction, learned correction, physics direction, promotion weight, and failure evidence. The example illustrates how PhyPro exposes the mechanism behind the update: the final correction is not a black-box residual, but a combination of a learned correction and a physics-aligned promotion term.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/figure_phypro_case.png}
  \caption{Representative PhyPro reconstruction case on PEMS-BAY under random missing with 50\% missingness. PhyPro reduces masked MAE from 0.2457 to 0.2070 in this case.}
...
PhyPro is lightweight because it operates on local evidence after the backbone output is produced. Table~\ref{tab:complexity} reports the additional trainable parameters and forward time. The module adds 23,365 parameters and less than 2.6 ms per batch on the tested datasets.

\begin{table}[t]
\centering
\small
\caption{Complexity overhead of PhyPro. Runtime is extra forward time per batch with shape $16{\times}12{\times}N{\times}1$.}
\label{tab:complexity}
\begin{tabular}{lccc}
\toprule
Dataset & Nodes & Extra params & Extra time (ms) \\
\midrule
PEMS03 & 358 & 23,365 & 2.52 \\
PEMS04 & 307 & 23,365 & 1.93 \\
PEMS08 & 170 & 23,365 & 0.99 \\
PEMS-BAY & 325 & 23,365 & 2.11 \\
METR-LA & 207 & 23,365 & 1.21 \\
\bottomrule
\end{tabular}
\end{table}
...
The anonymized implementation and reproduction scripts are available at: \url{https://anonymous.4open.science/r/PhyGuard-F1CE}.
